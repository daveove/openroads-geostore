window.workspace_groups = new Object();
                            ...
                            $("<div>", { class: "modal-body" }).html(
                                $("<div>", { class: "form-group" }).html(
                                    ...
                                    $("<div>", { class: "col-xs-7" }).html(
                                        $("<input>", { class: "form-control text-uppercase", id: "teamName", name: "workspace_name", placeholder: "Workspace Name", type: "text", required: "" })))).add(
                                $("<div>", { class: "form-group" }).html(
                                    $("<label>", { class: "control-label col-xs-4", for: "teamDescription", text: "Workspace Description " }).append(
                                        $("<span>", { class: "text-danger", text: "*"})).add(
                                    $("<div>", { class: "col-xs-7" }).html(
                                        $("<textarea>", { class: "form-control", id: "teamDescription", name: "workspace_description", placeholder: "A short description about the workspace", rows: "5", required: "" }))))).add(
                                $("<div>", { class: "form-group" }).html(
                                    $("<label>", { class: "control-label col-xs-4", for: "teamDescription", text: "User Groups " }).add(
                                    $("<div>", { class: "col-xs-7", id: 'userGroupWorkspace' }).html(
                                        $("<button>", { class: "btn btn-sm btn-success", type: 'button', id: "addUserGroupBtn", onclick: 'addUserGroupToNewWorkspace(this);', text: 'Add a User Group' }))))).add(
                                $("<div>", { class: "form-group" }).html(
                                    $("<label>", { class: "control-label col-xs-4", for: "inviteMembers", text: "Add Members " }).append(
                                        $("<span>", { class: "text-danger", text: "*"})).add(
                                    $("<div>", { class: "col-xs-7" }).html(
                                        $("<textarea>", { class: "form-control", id: "inviteMembers", name: "workspace_member_emails", placeholder: "User emails separated by comma", rows: "3", required: ""}).add(
...
function addUserGroupToNewWorkspace(obj){
                ...
                if(data){
                    ...
                    if(data.data.length > 0 ){
                        ...
                        for(i=0;i<data.data.length;i++){
                            if($('#userGroupWorkspace').find('p[data-group-id='+data.data[i].id+']').length == 0){
...
function updateWorkspace(obj){
    if(!$(obj).data("workspace-id")){
        /* Workspace ID is required when updating an workspace */
        ...
        success: function(data){
            ...
            if(data){
                ...
                for(i=0;i<data.user_groups_list.length;i++){
                    window.workspace_groups[data.user_groups_list[i].id] = data.user_groups_list[i]
                }
                $("body").append(
                    $("<div>", { class: "modal fade", id: "updateTeamModal", "tabindex": "-1", "role": "dialog", "aria-labelledby": "updateTeamModalLabel", "aria-hidden": "true" }).html(
                        $("<div>", { class: "modal-dialog" }).html(
                            $("<form>", { class: "form-horizontal", method: "POST", action: "/workspace/" + workspaceID, id: "updateTeamForm" }).html(
                                $("<div>", { class: "modal-content" }).html(
                                    $("<div>", { class: "modal-header" }).html(
                                        $("<button>", { class: "close", "data-dismiss": "modal", type: "button", "aria-hidden": "true", text: "×" }).add(
                                        ...
                                        $("<div>", { class: "modal-body" }).html(
                                            $("<div>", { class: "form-group" }).html(
                                                $("<label>", { class: "control-label col-xs-4", for: "teamName", text: "Workspace Name " }).append(
                                                    $("<span>", { class: "text-danger", text: "*"})).add(
                                                $("<div>", { class: "col-xs-7" }).html(
                                                    $("<input>", { class: "form-control", id: "teamName", name: "workspace_name", placeholder: "Workspace Name", type: "text", value: (data.title || ""), required: "" })))).add(
                                            $("<div>", { class: "form-group" }).html(
                                                $("<label>", { class: "control-label col-xs-4", for: "teamDescription", text: "Workspace Description " }).append(
                                                    $("<span>", { class: "text-danger", text: "*"})).add(
                                                $("<div>", { class: "col-xs-7" }).html(
                                                    $("<textarea>", { class: "form-control", id: "teamDescription", name: "workspace_description", placeholder: "A short description about the team", rows: "5", required: "", text: (data.description || "") }))))).add(
                                            $("<div>", { class: "form-group" }).html(
                    ...
                    for(i=0;i<data.members.length;i++){
                        $("#teamMembersContainer").append(
                            ...
                            $("<div>", { class: "col-xs-10 no-padding-left" }).html(
                                ...
                                $("<button>", { class: "btn btn-xs btn-danger pull-right " + (data.members[i].id == userID ? "remove-own-" + userID : "" ), type: "button", "data-team-id": workspaceID, 'data-user-name': data.members[i].name.toUpperCase(), 'data-workspace-name': data.title, "data-user-id": data.members[i].id, onclick: "removeMember(this);", text: " Remove User", disabled: (data.members[i].id == userID ? true : false )})).append(
                ...
                // if(window.workspace_groups){
                //     for(key in window.workspace_groups){
                //         if(window.workspace_groups[key].members.length > 0){
                    ...
                    for(i=0;i<data.user_groups_list.length;i++){
                        $("#userGroupsContainer").append(
                            $("<p>", { class: "form-control-static", text: data.user_groups_list[i].title }).append(
                                $('<button>', { type: 'button', class: 'btn btn-xs btn-danger pull-right', 'data-workspace-id': workspaceID, 'data-group-id': data.user_groups_list[i].id, text: ' Remove User Group', onclick: 'removeUserGroupFromEnv(this);' })));
                    }
                    $("#userGroupsContainer").append(
                            $("<button>", { class: "btn btn-sm btn-success", type: "button", "data-workspace-id": workspaceID, 'data-workspace-name': data.title, onclick: "addUserGroup(this);", text: " Add User Group"}));
                }
                else{
                ...
                if(data.invited_users.length>0){
                    ...
                    for(i=0;i<data.invited_users.length;i++){
                        $("#teamInvitedUsersContainer").append($("<p>", { class: "form-control-static", text: data.invited_users[i] }).append($("<button>", { class: "btn btn-xs btn-danger pull-right", type: "button", "data-team-id": workspaceID, "data-user-email": data.invited_users[i], onclick: "cancelInvite(this);", text: " Cancel Invite"})));
                    }
                    $("#teamInvitedUsersContainer").append($("<p>", { class: "form-control-static" }).html($("<button>", { class: "btn btn-sm btn-success", type: "button", "data-team-id": workspaceID, onclick: "inviteUsers(this);", text: " Invite Users"})));
                }
                else{
                    $("#teamInvitedUsersContainer").html($("<p>", { class: "form-control-static" }).html($("<button>", { class: "btn btn-sm btn-success", type: "button", "data-team-id": workspaceID, onclick: "inviteUsers(this);", text: " Invite Users"})));
                }
                ...
                $("#updateTeamModal").modal({ show: true, backdrop: "static", keyboard: false }).on("shown.bs.modal", function (){
                    $("#updateTeamForm").submit(function (e) {
                        ...
                        $.ajax({
                            url: "/workspace/" + workspaceID,
                            type: "POST",
                            data: {action: "update_team", workspace_name: $("#teamName").val(), workspace_description: $("#teamDescription").val(), visibility: $("input[type='radio'][name='visibility']:checked").val(), token: token},
                            success: function (data){
                                ...
                                if(data){
                                    if(data.code == 200){
                                        if(!data.success){
                                            ...
                                            $("."+workspaceID).text(data.data.title);
                                        }
                                        $(this).find("input.form-control:first").focus();
                                        return
                                    }
...
function removeMember(obj){
    ...
    workspaceName = $(obj).data('workspace-name');
    if(confirm('WARNING: You are about to remove '+userName+' from the '+workspaceName+' workspace, do you want to continue?')){
        $(obj).prepend($("<i>", { class: "fa fa-fw fa-spin fa-circle-o-notch" })).prop("disabled", true);
        $.ajax({
            url: "/workspace/" + teamID,
            type: "POST",
...
function removeUserGroup(obj){
    ...
    $("body").append(
        $("<div>", { class: "modal fade", id: "removeUserGroupModal", "tabindex": "-1", "role": "dialog", "aria-labelledby": "inviteUsersModalLabel", "aria-hidden": "true" }).html(
            $("<div>", { class: "modal-dialog" }).html(
                $("<form>", { class: "form-horizontal", method: "POST", action: "/workspace/" + workspaceID, id: "removeUserGroupForm" }).html(
                    $("<div>", { class: "modal-content" }).html(
                        $("<div>", { class: "modal-header" }).html(
                            $("<button>", { class: "close", "data-dismiss": "modal", type: "button", "aria-hidden": "true", text: "×" }).add(
                            $("<h3>", { class: "modal-title", id: "inviteUsersModalLabel", text: "Remove User Group from " + workspaceName }))
                        ).add(
                            $("<div>", { class: "modal-body" }).html($('<p>', { class: 'text-center'}).html($('<i>', { class: 'fa fa-fw fa-3x fa-spin fa-circle-o-notch' })).add(
            $('<h4>', { class: 'text-center', text: 'Loading user groups...' })))
                        ).add(
    ...
    $("#removeUserGroupModal").modal({ show: true, backdrop: "static", keyboard: false }).on('shown.bs.modal', function (){
        $.ajax({
            url: '/groups?fetch=all&removeusergroup=true&workspaceid=' + workspaceID,
            success: function (data){
                if(data.length > 0){
...
function addUserGroup(obj){
    ...
    $("body").append(
        $("<div>", { class: "modal fade", id: "addUserGroupModal", "tabindex": "-1", "role": "dialog", "aria-labelledby": "inviteUsersModalLabel", "aria-hidden": "true" }).html(
            $("<div>", { class: "modal-dialog" }).html(
                $("<form>", { class: "form-horizontal", method: "POST", action: "/workspace/" + workspaceID, id: "addUserGroupForm" }).html(
                    $("<div>", { class: "modal-content" }).html(
                        $("<div>", { class: "modal-header" }).html(
                            $("<button>", { class: "close", "data-dismiss": "modal", type: "button", "aria-hidden": "true", text: "×" }).add(
                            $("<h3>", { class: "modal-title", id: "inviteUsersModalLabel", text: "Add User Group to " + workspaceName }))
                        ).add(
                            $("<div>", { class: "modal-body" }).html($('<p>', { class: 'text-center'}).html($('<i>', { class: 'fa fa-fw fa-3x fa-spin fa-circle-o-notch' })).add(
            $('<h4>', { class: 'text-center', text: 'Loading user groups...' })))
                        ).add(
    ...
    $("#addUserGroupModal").modal({ show: true, backdrop: "static", keyboard: false }).on('shown.bs.modal', function (){
        $.ajax({
            url: '/groups?fetch=all&addusergroup=true&workspaceid=' + workspaceID,
            success: function (data){
                if(data.length > 0){
                    $('#addUserGroupModal').find('div.modal-body').html(
                        $('<table>', { class: 'table table-striped', id: 'table-user-group-list' }).html(
                        ...
                        if(data.code == 200){
                            if(!data.success){
                                ...
                                if(data.workspace['members'].length > 0){
                                    $("#teamMembersContainer").empty();
                                    for(i=0;i<data.workspace['members'].length;i++){
                                        $("#teamMembersContainer").append(
                                            $("<div>", { class: "row", id: data.workspace['members'][i].id + "_update" }).html(
                                            $("<div>", { class: "col-xs-2 no-padding-right" }).html(
                                                $("<img>", { class: "navbar-avatar", src: "https://www.gravatar.com/avatar/" + data.workspace['members'][i].email_md5 + "?s=30", style: "padding-top: 10px;" })).add(
                                            $("<div>", { class: "col-xs-10 no-padding-left" }).html(
                                                $("<p>", { class: "form-control-static", text: data.workspace['members'][i].name + (data.workspace['members'][i].id == userID ? " (YOU)" : "" ) }).append(
                                                $("<button>", { class: "btn btn-xs btn-danger pull-right " + (data.workspace['members'][i].id == userID ? "remove-own-" + userID : "" ), type: "button", "data-team-id": workspaceID, 'data-user-name': data.workspace['members'][i].name.toUpperCase(), 'data-workspace-name': data.title, "data-user-id": data.workspace['members'][i].id, onclick: "removeMember(this);", text: " Remove User", disabled: (data.workspace['members'][i].id == userID ? true : false )})).append(
                                                $("<br>")).append(" (" + data.workspace['members'][i].email + ")")))));
                                    }
                                }
                                else{
                                    $("#teamMembersContainer").html($("<p>", { class: "form-control-static", text: " No team members" }).prepend($("<i>", { class: "fa fa-fw fa-info"})));
                                ...
                                for(i=0;i<data.user_groups.length;i++){
                                    $("#userGroupsContainer").prepend(
                                        $("<p>", { class: "form-control-static", text: data.user_groups[i].title }).append(
                                            $('<button>', { type: 'button', class: 'btn btn-xs btn-danger pull-right', text: ' Remove User Group', 'data-workspace-id': workspaceID, 'data-group-id': data.user_groups[i].id, onclick: 'removeUserGroupFromEnv(this);' })));
                                }
                                $('#addUserGroupModal').modal('hide');
                            }
                            $(this).find("input.form-control:first").focus();
